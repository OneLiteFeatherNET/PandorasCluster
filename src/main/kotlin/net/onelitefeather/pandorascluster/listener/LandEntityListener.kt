...
class LandEntityListener(private val landService: LandService) : Listener, EntityUtils {
    ...
    fun theConfuser(event: EntityDamageByEntityEvent) {
        ...
        if (attacker is Player) {
            event.isCancelled = if (target !is Player) {
                val flag = landService.getLandFlag(LandFlag.PVE, land) ?: return
                val value = flag.getValue<Boolean>()
                hasPermission(attacker, LandFlag.PVE) || value == false
            } else {
                val flag = landService.getLandFlag(LandFlag.PVP, land) ?: return
                val value = flag.getValue<Boolean>()
    ...
    fun handleEntityMount(event: EntityMountEvent) {
        ...
        if (hasPermission(entity, LandFlag.ENTITY_MOUNT)) return
        event.isCancelled = true
    }
        ...
        if (projectile is ThrownPotion) {
            ...
            if (projectileShooter is Entity) {
                ...
                if (land != null) {
                    ...
                    if (hasPermission(projectileShooter, LandFlag.POTION_SPLASH)) return
                    event.isCancelled = true
                }
            }
        }
    ...
    fun handleVehicleEnter(event: VehicleEnterEvent) {
        ...
        if (land != null) {
            ...
            if (hasPermission(entered, LandFlag.VEHICLE_USE)) return
            event.isCancelled = true
        }
    }
}